...
void Multi_SubSampling(struct EqualRotationAxis * eraxis ,const User_IMU_t * Imu,const float dt ,Cal_Method CM, enum Sample sample_state)
{
        ...
        switch (sample_state) {
                ...
                case SimpleandLast:
                        ...
                        PhitoEular(eraxis->phi, eraxis->Q_btoi, glv_ins.yaw_pitch_roll,&eraxis->C, CM);
                        break;
...
void Velocity_Update(struct Velocity * v, struct EqualRotationAxis * eraxis, const User_IMU_t * Imu,const float dt)
{
        ...
        QuaterniontoConsine(eraxis->Q_btoi, eraxis->C.C_btoi);
...
void PhitoEular(float phi[3], float Q_btoi[4], float angle[3], struct CosineMatrix *C, enum Cal_Method Cm)
{
        switch (Cm) {
                case Quaternion:
                        EquivalentRotationVectortoQuaternions(phi,Q_btoi);
                        QuaterniontoEular(Q_btoi, angle);
                break;

                case DirectionCosineMatrix:
                        M_equal_Rotation_vector(phi, &C->sC_btob_minus);

                        //arm_mat_mult_f32(&seye3,&C->sC_btoi,&C->sC_btoi);
                        arm_mat_mult_f32(&C->sC_btoi,&C->sC_btob_minus,&C->sC_btoi);
                        AttitudeMatrixtoEular(C->C_btoi,angle);
                break;
        }
}
 ...
 * @note 里面的变换因为我懒得写到外面了，旋转矩阵的变换就顺带做了
 */
void EquivalentRotationVectortoQuaternions(const float phi3[3],float * Q_btoi)
{
        ...
        Q_btob_minus[0] = cosf(abs_phi/2.0f);
        Q_btob_minus[1] = phi3[0] / abs_phi * sinf(abs_phi/2.0f);
        Q_btob_minus[2] = phi3[1] / abs_phi * sinf(abs_phi/2.0f);
        Q_btob_minus[3] = phi3[2] / abs_phi * sinf(abs_phi/2.0f);
        //这一步是因为我懒得写到外面了，旋转矩阵的变换就顺带做了
        multiplyQuaternions(Q_btoi, Q_btob_minus,Q_btoi);
        dstatic(float, abs);
